...
public class User {
	...
	public static User makeOpponent(int lv) {
		...
		return new User(RANDOM_NAMES[seed], seed, 5 + atkBonus, 2 + defBonus, expBonus, lv, 10 + hpBonus, spBonus, 50,
				0);
	}
	...
	public void echoStats() {
		...
		Engine.echo("Build: "
				+ (build == Build.OFFENSIVE ? "OFFENSIVE" : (build == Build.DEFENSIVE ? "DEFENSIVE" : "MAGICAL")));
	...
	private static User login(String user) {
		...
		if (accountExists) {
			// load stats in
			try (Scanner stat = new Scanner(accountFile)) {
				// rehydrate User
